...
一个用于在 Android arm64 设备上**原生运行 [OpenList](https://github.com/OpenListTeam/OpenList)** 的 Magisk 模块。

## 功能特性

- **开机自启**：开机后自动启动 OpenList，无需手动干预
- **进程守护**：内置看门狗，每 60 秒检查一次，OpenList 异常退出自动拉起
...
- **原生 arm64**：内置官方 `linux-arm64` 二进制，无需依赖 Termux
...
1. 下载 [magisk_openlist.zip](./magisk_openlist.zip) 或自行打包
...
模块安装后，在 Magisk 模块页面的 **Openlist_online** 卡片底部会显示 **"操作"** 按钮。
...
- **关闭模块**：立即终止 OpenList 进程，释放 5244 端口，无需重启
- **打开模块**：立即启动 OpenList 进程，恢复 5244 端口监听，无需重启
- 模块介绍区域会实时显示当前状态（`运行中` / `已停止`）

### 看门狗机制

- 手动关闭模块后，看门狗暂停守护（不会自动重启）
- 手动打开模块后，看门狗恢复守护（OpenList 异常退出会自动拉起）
- 完全无需重启即可切换状态
...
开机
  ...
  │     └─ sh start_openlist.sh &
  └─ Magisk 触发 post-fs-data.sh   （alpha 版 Magisk 兜底）
        └─ 等 sys.boot_completed=1
        └─ sh start_openlist.sh

start_openlist.sh（开机流程）:
  1. setup_env（查找 busybox，设置 setsid）
  2. init_module（一次性初始化：MODDIR/锁文件/wake_lock/data目录/admin密码）
  3. launch_openlist（启动 openlist，更新状态描述为"运行中"）
  4. run_watchdog（看门狗循环，检查 .paused 标记）

操作流程:
  点击"操作"按钮 → action.sh
    ├─ 运行中: 音量上 → start_openlist.sh stop（创建 .paused + kill 进程 + 更新描述）
    └─ 已停止: 音量上 → start_openlist.sh launch（删除 .paused + 启动进程 + 更新描述）
```

## 卸载方法

1. Magisk App → 模块 → Openlist_online → **移除**
2. 重启手机
3. 卸载脚本会自动：
   - 释放 wake_lock
   - 清理残留备份目录
...
**Q: 浏览器打不开 5244？**
- 等 30 秒让开机流程跑完
- 用 `pgrep -af openlist` 检查进程是否在跑
- 查看 `/data/adb/modules/Openlist_online/openlist.log` 排错
...
- [OpenListTeam/OpenList](https://github.com/OpenListTeam/OpenList) — OpenList 原项目（AList 社区分支）
- [topjohnwu/Magisk](https://github.com/topjohnwu/Magisk) — Magisk 框架

## 许可

遵循 OpenList 原项目 AGPL 3.0 许可。
